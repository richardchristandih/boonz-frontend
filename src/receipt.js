...
function alignLeft()  { return ESC + "a" + "\x00"; }
function alignCenter(){ return ESC + "a" + "\x01"; }
function alignRight() { return ESC + "a" + "\x02"; }
function boldOn()     { return ESC + "E" + "\x01"; }
function boldOff()    { return ESC + "E" + "\x00"; }
function dblOn()      { return GS  + "!" + "\x11"; } // double h/w
function dblOff()     { return GS  + "!" + "\x00"; }
function init()       { return ESC + "@"; }
function cut()        { return GS  + "V" + "\x41" + "\x10"; } // partial cut
...
export function buildReceipt({
  shopName, address, orderNumber, dateStr,
  items = [], subtotal = 0, tax = 0, service = 0, discount = 0,
  total = 0, payment, orderType, customer
}) {
  const w = 32; // 58mm ~32 chars (80mm ~42)
  let out = "";

  out += init();
  out += alignCenter() + boldOn() + dblOn() + line(shopName || "");
  out += dblOff() + boldOff();
  ...
  for (const it of items) {
    ...
    out += alignLeft() + line(lr(`  ${qty} x Rp.${price.toFixed(2)}`, `Rp.${lineTotal.toFixed(2)}`, w));
  }

  out += line("--------------------------------");
  out += alignLeft() + line(lr("Subtotal", `Rp.${(+subtotal).toFixed(2)}`, w));
  out += alignLeft() + line(lr("Tax (10%)", `Rp.${(+tax).toFixed(2)}`, w));
  out += alignLeft() + line(lr("Service (5%)", `Rp.${(+service).toFixed(2)}`, w));
  if (+discount > 0) out += alignLeft() + line(lr("Discount", `-Rp.${(+discount).toFixed(2)}`, w));

  out += line("--------------------------------");
  out += alignLeft() + boldOn() + line(lr("TOTAL", `Rp.${(+total).toFixed(2)}`, w)) + boldOff();
